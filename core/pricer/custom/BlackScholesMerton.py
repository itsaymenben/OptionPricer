...
class BlackScholesMertonPricer(BasePricer):
    ...
    def _compute_call_price(self, d1, d2):
        return self.start_price * norm.cdf(d1) - self.strike_price * np.exp(- self.risk_free_rate * self.time_to_maturity) * norm.cdf(d2)

    def _compute_put_price(self, d1, d2):
        return self.strike_price * np.exp(- self.risk_free_rate * self.time_to_maturity) * norm.cdf(- d2) - self.start_price * norm.cdf(- d1)
