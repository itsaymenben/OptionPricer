...
class TrinomialTreePlotter(BasePlotter):
    ...
    def generate_plot(self):
        ...
        y_asset_prices = [j - i + self.NODE_OFFSET for (i, j, _) in self.asset_prices_nodes]
        y_option_price = [j - i - self.NODE_OFFSET for (i, j, _) in self.asset_prices_nodes]
        ...
        fig.update_layout(
            title='Trinomial Tree for Option Pricing',
            showlegend=False,
    ...
    def _build_nodes_and_edges(self):
        ...
        for i in range(self.n_steps + 1):
            for j in range(2 * i + 1):
                self.asset_prices_nodes.append((i, j, self.asset_prices[i][2 * i - j]))
                self.option_prices_nodes.append((i, j, self.option_prices[i][2 * j - j]))
                if i <= self.n_steps - 1:
